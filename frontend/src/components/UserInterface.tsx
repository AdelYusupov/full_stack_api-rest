...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:1010';
    ...
    const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try{
            ...
            setUsers(
                users.map((users) => {
                    if (users.id == parseInt(updateUser.id)){
                        return { ...users, name: updateUser.name, email: updateUser.email};
                    }
                    return users;
                })
            );
    ...
    return (
        <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
            <img src={`/${backendName}logo.jpg`} alt={`${backendName} Logo`} className="w-20 h-20 mb-6 mx-auto"/>
            ...
            {/* Create user */}
            <form onSubmit={createUser} className="mb-6 p-4 bg-blue-100 roinded shadow">
                <input
                    placeholder="Name"
                    value={newUser.name}
            ...
            <div className="space-y-4">
                {users.map((user) => (
                    <div key={user.id} className="flex items-center justify-between bg-white p-4 rounde-lg shadow">
